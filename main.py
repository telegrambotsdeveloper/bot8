...
async def referral_cb(callback: CallbackQuery):
    ...
    f"Приглашайте друзей по вашей ссылке и зарабатывайте бонусные ⭐!\n"
    f"— За каждую первую покупку приглашённого вы получите бонус ⭐.\n"
    f"— Чем больше друзей — тем больше бонусов!\n\n"
    f"Ваша ссылка для приглашений:\n"
    f"<code>/start ref_{user_id}</code>\n\n"
    f"Отправьте её друзьям или разместите в соцсетях.\n"
    f"👥 Приглашённых: {len(referrals)}"
)
    await callback.message.answer(text, parse_mode="HTML")
    else:
...
async def how_it_works(callback: CallbackQuery):
    ...
    text = (
        ...
        "1️⃣ Вы получаете 1 бесплатный токен при входе в бота.\n"
        "2️⃣ Ещё 1 токен — за подписку на канал.\n"
        "3️⃣ 10⭐ = 1 токен. ⭐ можно купить прямо в боте.\n"
        "4️⃣ 1 прогноз = списание токенов (стоимость зависит от модели).\n\n"
        "💡 Пример:\n"
        "Вы выбрали матч 'Манчестер Сити — Ливерпуль', бот даст прогноз: победитель, счёт, аргумент.\n\n"
        "🤝 Реферальная программа: за каждого приглашённого, который купит токены, вы получите бонус ⭐."
    )
    await callback.message.answer(text, parse_mode="HTML")")
